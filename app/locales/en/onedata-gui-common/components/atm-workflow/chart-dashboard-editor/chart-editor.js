import elements from './chart-editor/elements';
import seriesList from './chart-editor/series-list';
import seriesGroupsList from './chart-editor/series-groups-list';
import seriesGroupsListItem from './chart-editor/series-groups-list-item';
import axesList from './chart-editor/axes-list';
import elementsListItem from './chart-editor/elements-list-item';
import preview from './chart-editor/preview';
import elementsEditor from './chart-editor/elements-editor';
import seriesEditor from './chart-editor/series-editor';
import seriesEditorForm from './chart-editor/series-editor-form';
import seriesGroupEditor from './chart-editor/series-group-editor';
import seriesGroupEditorForm from './chart-editor/series-group-editor-form';
import axisEditor from './chart-editor/axis-editor';
import axisEditorForm from './chart-editor/axis-editor-form';

export default {
  elements,
  seriesList,
  seriesGroupsList,
  seriesGroupsListItem,
  axesList,
  elementsListItem,
  preview,
  elementsEditor,
  seriesEditor,
  seriesEditorForm,
  seriesGroupEditor,
  seriesGroupEditorForm,
  axisEditor,
  axisEditorForm,
};

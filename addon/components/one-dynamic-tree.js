...
export default Ember.Component.extend(ValuesTree, FieldsTree, CheckboxSelectionTree, {
  layout,
   ...
   * @type {Array.Object}
   */
  _errors: computed('_fieldsTree', 'validations.errors.[]', function () {
    let {
      _fieldsTree,
      validations
    ...
    return validations.get('errors').filter((error) =>
      !!_fieldsTree.get(error.get('attribute').substring('values.'.length))
    );
  }),
  ...
  _fieldsTreeObserver: on('init', observer('_fieldsTree', function () {

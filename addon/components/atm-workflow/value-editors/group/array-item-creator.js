/**
 * An array item creator dedicated for creating group items.
 *
...
import layout from 'onedata-gui-common/templates/components/atm-workflow/value-editors/group/array-item-creator';
import ArrayItemCreatorBase from '../commons/array-item-creator-base';

export default ArrayItemCreatorBase.extend(I18n, {
  layout,
  classNames: ['group-array-item-creator'],
  classNameBindings: ['isHiddenDueToDisabled:hidden'],

  /**
   * @override
   */
  i18nPrefix: 'components.atmWorkflow.valueEditors.group.arrayItemCreator',

  /**
   * @type {Utils.AtmWorkflow.ValueEditors.ValueEditorStates.Group | null}
   */
  ...
  handleIdProvidingEditorChange() {
    ...
    switch (this.idProvidingEditorState.mode) {
      case 'selected': {
        const group = this.idProvidingEditorState.value;
        const newItemEditorState =
          this.stateManager.createValueEditorState(this.itemAtmDataSpec, group);
  ...
  actions: {
    groupsSelected(selectedGroups) {
      if (!this.stateManager || !selectedGroups.length) {
        return;
      }

      const newItemEditorStates = selectedGroups.map((group) =>
        this.stateManager.createValueEditorState(this.itemAtmDataSpec, group)
      );

...
describe('Unit | Utility | one-time-series-chart/format-value-with-unit', function () {
  ...
  testFormatValueWithUnit(1111.1111, '1 111.111');
  ...
  testFormatValueWithUnit(-1024, '-1 KiB', 'bytes');
  testFormatValueWithUnit(1111.1111, '1.1 KiB', 'bytes');
  ...
  testFormatValueWithUnit(-1000, '-1 kB', 'bytes', { format: 'si' });
  testFormatValueWithUnit(1111.1111, '1.1 kB', 'bytes', { format: 'si' });
  ...
  testFormatValueWithUnit(-1024, '-1 Kib', 'bits');
  testFormatValueWithUnit(1111.1111, '1.1 Kib', 'bits');
  ...
  testFormatValueWithUnit(-1000, '-1 kb', 'bits', { format: 'si' });
  testFormatValueWithUnit(1111.1111, '1.1 kb', 'bits', { format: 'si' });
  ...
  testFormatValueWithUnit(-1024, '-1 KiB/s', 'bytesPerSec');
  testFormatValueWithUnit(1111.1111, '1.1 KiB/s', 'bytesPerSec');
  ...
  testFormatValueWithUnit(-1000, '-1 kB/s', 'bytesPerSec', { format: 'si' });
  testFormatValueWithUnit(1111.1111, '1.1 kB/s', 'bytesPerSec', { format: 'si' });
  ...
  testFormatValueWithUnit(-1024, '-1 Kib/s', 'bitsPerSec');
  testFormatValueWithUnit(1111.1111, '1.1 Kib/s', 'bitsPerSec');
  ...
  testFormatValueWithUnit(-1000, '-1K c/s', 'countsPerSec');
  testFormatValueWithUnit(1111.1111, '1.111K c/s', 'countsPerSec');
  ...
  testFormatValueWithUnit(-1000, '-1K ops/s', 'operationsPerSec');
  testFormatValueWithUnit(1111.1111, '1.111K ops/s', 'operationsPerSec');
  ...
  testFormatValueWithUnit(-1000, '-1K req/s', 'requestsPerSec');
  testFormatValueWithUnit(1111.1111, '1.111K req/s', 'requestsPerSec');
  ...
  testFormatValueWithUnit(-1000, '-1K rd/s', 'readsPerSec');
  testFormatValueWithUnit(1111.1111, '1.111K rd/s', 'readsPerSec');
  ...
  testFormatValueWithUnit(-1000, '-1K wr/s', 'writesPerSec');
  testFormatValueWithUnit(1111.1111, '1.111K wr/s', 'writesPerSec');
  ...
  testFormatValueWithUnit(-1000, '-1K io/s', 'ioOperationsPerSec');
  testFormatValueWithUnit(1111.1111, '1.111K io/s', 'ioOperationsPerSec');
  ...
  testFormatValueWithUnit(1111.1111, '1 111.111%', 'percent');

  testFormatValueWithUnit(0, '0%', 'percentNormalized');
  testFormatValueWithUnit(100, '10 000%', 'percentNormalized');
  testFormatValueWithUnit(0.5, '50%', 'percentNormalized');
  testFormatValueWithUnit(-1, '-100%', 'percentNormalized');
  testFormatValueWithUnit(11.111111, '1 111.111%', 'percentNormalized');
  ...
  testFormatValueWithUnit(1000, '1 000', 'custom');
  testFormatValueWithUnit(0.5, '0.5', 'custom');
  testFormatValueWithUnit(-1, '-1', 'custom');
  testFormatValueWithUnit(1111.1111, '1 111.111', 'custom');
  ...
  testFormatValueWithUnit(1111.1111, '1.111K', 'custom', { useMetricSuffix: true });

  testFormatValueWithUnit(0, '0 km', 'custom', { customName: 'km' });
  testFormatValueWithUnit(1000, '1 000 km', 'custom', { customName: 'km' });
  testFormatValueWithUnit(0.5, '0.5 km', 'custom', { customName: 'km' });
  testFormatValueWithUnit(-1, '-1 km', 'custom', { customName: 'km' });
  testFormatValueWithUnit(1111.1111, '1 111.111 km', 'custom', { customName: 'km' });
  ...
  testFormatValueWithUnit(1111.1111, '1.111K km', 'custom', {
    useMetricSuffix: true,
    customName: 'km',
  });
});
...
function testFormatValueWithUnit(value, output, unitName, unitOptions) {
  ...
  it(`returns ${stringifiedOutput} for ${stringifiedInput} and ${stringifiedUnitName} unit${unitOptions ? ` options (${stringifiedUnitOptions})` : ''}`,
    function () {
      const funcInput = { value, unitName, unitOptions };
      const result = formatValueWithUnit(funcInput);
      const resultWithoutHtml = formatValueWithUnit({ ...funcInput, allowHtml: false });

      if (!result) {
        expect(result).to.deep.equal(output);
        expect(resultWithoutHtml).to.deep.equal(output);
      } else {
        expect(isHTMLSafe(result)).to.be.true;
        expect(
          String(result).replace(/<span class="thousand-space"><\/span>/g, ' ')

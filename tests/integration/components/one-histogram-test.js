...
describe('Integration | Component | one histogram', function () {
  setupComponentTest('one-histogram', {
    integration: true,
  });
...
async function render(testCase) {
  testCase.render(hbs `{{one-histogram configuration=configuration}}`);
  await wait();
}

function expectNoDataToShow(testCase) {
  expect(testCase.$('.one-histogram > *')).to.have.length(1);
  expect(testCase.$('.one-histogram')).to.have.class('no-data');

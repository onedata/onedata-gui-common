...
describe('Integration | Component | form component/static user field', function () {
  ...
  it(
    'renders only user icon and dash when user and field value are empty',
    async function () {
      await render(hbs `{{form-component/static-user-field field=field}}`);
  ...
  it(
    'renders user name from field.user property when field.value is empty',
    async function () {

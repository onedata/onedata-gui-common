...
describe('Integration | Component | atm-workflow/value-presenters/range/table-body-row-presenter', function () {
  setupRenderingTest();
  ...
  it('has classes "table-body-row-presenter" and "range-table-body-row-presenter"', async function () {
    ...
    expect(this.element.children[0]).to.have.class('table-body-row-presenter')
      .and.to.have.class('range-table-body-row-presenter');
  });
  ...
  it('shows three columns - start, end and step', async function () {
    ...
    expect(tds[0]).to.have.class('column-start')
      .and.to.have.trimmed.text('2');
    expect(tds[1]).to.have.class('column-end')
      .and.to.have.trimmed.text('10');
    expect(tds[2]).to.have.class('column-step')
      .and.to.have.trimmed.text('3');
  });
});
